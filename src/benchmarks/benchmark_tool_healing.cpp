...
#include "../inference/lfg_inference.h" // for lfg_tokenize
...
void run_benchmark(const std::string& model_path, bool enable_healing) {
    ...
    lfg_session_configure_structured(session, preamble_grammar.c_str(), "root");
    ...
    lfg_tokenize(lfg_model_get_vocab(model), prompt.c_str(), prompt.length(), tokens.data(), n_tokens, false, false);
    ...
    for (int i = 0; i < n_gen; ++i) {
        ...
        int n = lfg_token_to_piece(vocab, token, buf, sizeof(buf), 0, true); // true = render specials?
