...
using lfm_buf_map = std::unordered_map<uint32_t, ggml_backend_buffer_t>;
#endif
...
const char * lfm_file_version_name(enum lfm_fver version);

#ifdef __cplusplus
struct lfm_model_loader {
    // Holds information on a model weight
    struct lfm_tensor_weight {
        uint16_t  idx; // source file index
        size_t   offs; // tensor data offset in the original file

        ggml_tensor * tensor;

        lfm_tensor_weight(const lfm_file * file, uint16_t idx, const struct gguf_context * gguf_ctx, ggml_tensor * tensor) : idx(idx), tensor(tensor) {
    ...
    std::map<std::string, lfm_tensor_weight, weight_name_comparer> weights_map;
    std::unordered_map<std::string, lfm_model_kv_override> kv_overrides;
    const lfm_model_tensor_buft_override * tensor_buft_overrides;
    ...
    const lfm_tensor_weight * get_weight(const char * name) const;

    const lfm_tensor_weight & require_weight(const char * name) const;
    ...
    void init_mappings(bool prefetch = true, lfm_mlocks * mlock_mmaps = nullptr);
