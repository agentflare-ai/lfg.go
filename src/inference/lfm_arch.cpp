...
static std::set<llm_tensor> llm_get_tensor_names(llm_arch arch) {
    switch (arch) {
        ...
        default:
            lfm_set_last_error(LFM_ERROR_UNSUPPORTED, "%s: unknown architecture for tensor mapping", __func__);
            return {};
    }
}
...
std::string LLM_TN_IMPL::str() const {
    if (LLM_TENSOR_NAMES.find(tensor) == LLM_TENSOR_NAMES.end()) {
        lfm_set_last_error(LFM_ERROR_INVALID_ARGUMENT, "%s: unknown tensor id %d", __func__, static_cast<int>(tensor));
        return "";
    }
