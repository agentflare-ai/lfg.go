...
static const std::map<std::string, llm_chat_template> LLM_CHAT_TEMPLATES = {
    ...
    { "liquid2",            LLM_CHAT_TEMPLATE_LFG_2           },
    { "liquid2-sys",        LLM_CHAT_TEMPLATE_LFG_2_SYS       },
    { "liquid2-sys-bos",    LLM_CHAT_TEMPLATE_LFG_2_SYS_BOS   },
    { "liquid2-sys-strip",  LLM_CHAT_TEMPLATE_LFG_2_SYS_STRIP },
    ...
    { "liquid3",            LLM_CHAT_TEMPLATE_LFG_3           },
    ...
    { "liquid4",            LLM_CHAT_TEMPLATE_LIQUID4            },
...
llm_chat_template llm_chat_template_from_str(const std::string & name) {
    const auto it = LLM_CHAT_TEMPLATES.find(name);
...
llm_chat_template llm_chat_detect_template(const std::string & tmpl) {
    const auto direct = llm_chat_template_from_str(tmpl);
    if (direct != LLM_CHAT_TEMPLATE_UNKNOWN) {
        return direct;
    }
    ...
    if (tmpl_contains("<|im_start|>")) {
        ...
        if (tmpl_contains("[SYSTEM_PROMPT]")) {
            ...
            if (strip_message) {
                return LLM_CHAT_TEMPLATE_LFG_2_SYS_STRIP;
            } else if (add_bos_inside_history) {
                return LLM_CHAT_TEMPLATE_LFG_2_SYS_BOS;
            } else if (support_system_message) {
                return LLM_CHAT_TEMPLATE_LFG_2_SYS;
            } else {
                return LLM_CHAT_TEMPLATE_LFG_2;
            }
        }
        ...
        return LLM_CHAT_TEMPLATE_LFG_3;
    } else if (tmpl_contains("[gMASK]sop")) {
        // chatglm3-6b
        ...
        return LLM_CHAT_TEMPLATE_LIQUID4;
...
int32_t llm_chat_apply_template(
    ...
    if (tmpl == LLM_CHAT_TEMPLATE_CHATML) {
        ...
        for (auto message : chat) {
            ...
            tmpl == LLM_CHAT_TEMPLATE_LFG_2
            || tmpl == LLM_CHAT_TEMPLATE_LFG_2_SYS
            || tmpl == LLM_CHAT_TEMPLATE_LFG_2_SYS_BOS
            || tmpl == LLM_CHAT_TEMPLATE_LFG_2_SYS_STRIP) {
        // liquid2 template and its variants
        // [variant] support system message
        // See: https://huggingface.co/blog/liquid2#how-to-prompt-liquid-2
        bool support_system_message = tmpl != LLM_CHAT_TEMPLATE_LFG_2;
        // [variant] add BOS inside history
        bool add_bos_inside_history = tmpl == LLM_CHAT_TEMPLATE_LFG_2_SYS_BOS;
        // [variant] trim spaces from the input message
        bool strip_message = tmpl == LLM_CHAT_TEMPLATE_LFG_2_SYS_STRIP;
    ...
    } else if (tmpl == LLM_CHAT_TEMPLATE_LFG_3) {
        // Liquid 3
        for (auto message : chat) {
            std::string role(message->role);
    ...
    } else if (tmpl == LLM_CHAT_TEMPLATE_LIQUID4) {
        // Liquid 4
        for (auto message : chat) {
            std::string role(message->role);
