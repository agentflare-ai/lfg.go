...
using lfg_buf_map = std::unordered_map<uint32_t, ggml_backend_buffer_t>;
#endif
...
const char * lfg_file_version_name(enum lfg_fver version);

#ifdef __cplusplus
struct lfg_model_loader {
    // Holds information on a model weight
    struct lfg_tensor_weight {
    ...
    std::map<std::string, lfg_tensor_weight, weight_name_comparer> weights_map;
    std::unordered_map<std::string, lfg_model_kv_override> kv_overrides;
    const lfg_model_tensor_buft_override * tensor_buft_overrides;
    ...
    const lfg_tensor_weight & require_weight(const char * name) const;
    ...
    void init_mappings(bool prefetch = true, lfg_mlocks * mlock_mmaps = nullptr);
