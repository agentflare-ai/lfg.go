...
// Redirect native printf/fprintf to GGML_LOG
#define printf(...) GGML_LOG_DEBUG(__VA_ARGS__)
#define fprintf(stream, ...) \
    do { \
        if ((stream) == stderr) { GGML_LOG_ERROR(__VA_ARGS__); } \
        else if ((stream) == stdout) { GGML_LOG_DEBUG(__VA_ARGS__); } \
        else { (fprintf)((stream), __VA_ARGS__); } \
    } while (0)
