...
struct lfg_build_modern_bert : public lfg_graph_context {
    const lfg_model & model;

    inline lfg_build_modern_bert(const lfg_model & model, const lfg_graph_params & params) :
        ...
        // embed layer norm
        inpL = build_norm(inpL, model.tok_norm, nullptr, LFG_NORM, -1);
        cb(inpL, "inp_norm", -1);
        ...
        if (hparams.pooling_type == LFG_POOLING_TYPE_CLS) {
