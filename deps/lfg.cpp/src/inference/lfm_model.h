...
struct lfm_layer {
    ...
    struct lfm_layer_posnet posnet;

    struct lfm_layer_convnext convnext;

    struct lfm_layer_shortconv shortconv;

    struct lfm_layer_nextn nextn;
};
...
struct lfm_model {
    ...
    std::unordered_set<lfm_adapter_lora *> loras;

    int64_t t_load_us  = 0;
    int64_t t_start_us = 0;

    explicit lfm_model(const struct lfm_model_params & params);
    ~lfm_model();

    void load_stats  (lfm_model_loader & ml);
    void load_arch   (lfm_model_loader & ml);
    void load_hparams(lfm_model_loader & ml);
    void load_vocab  (lfm_model_loader & ml);
    bool load_tensors(lfm_model_loader & ml); // returns false if cancelled by progress_callback
    ...
    lfm_split_mode split_mode() const;
    ...
    float get_rope_freq_base (const lfm_cparams & cparams, uint32_t il) const;
    float get_rope_freq_scale(const lfm_cparams & cparams, uint32_t il) const;

    ggml_tensor * get_rope_factors(const lfm_cparams & cparams, uint32_t il) const;

    // TODO: move this to new llm_arch_model_i interface
    lfm_memory_i * create_memory(const lfm_memory_params & params, const lfm_cparams & cparams) const;

    // TODO: move this to new llm_arch_model_i interface
    ggml_cgraph * build_graph(const llm_graph_params & params) const;

private:
    lfm_model_params params;
