...
// utilizes instances of lfg_memory_recurrent and lfg_kv_cache to
//   support models where each layer may be either attention-based or recurrent

class lfg_memory_hybrid : public lfg_memory_i {
public:
    lfg_memory_hybrid(
    ...
    //
    // lfg_memory_hybrid specific API
    //

    lfg_kv_cache * get_mem_attn() const;
    lfg_memory_recurrent * get_mem_recr() const;

private:
    const lfg_hparams & hparams;

    const std::unique_ptr<lfg_kv_cache> mem_attn;
    const std::unique_ptr<lfg_memory_recurrent> mem_recr;
};

class lfg_memory_hybrid_context : public lfg_memory_context_i {
public:
    using slot_info_vec_t = lfg_kv_cache::slot_info_vec_t;

    // init failure
    explicit lfg_memory_hybrid_context(lfg_memory_status status);

    // init full
    explicit lfg_memory_hybrid_context(lfg_memory_hybrid * mem);

    // init update
    explicit lfg_memory_hybrid_context(
        lfg_memory_hybrid * mem,
              lfg_context * lctx,
                       bool   optimize);

    // init success
    lfg_memory_hybrid_context(
              lfg_memory_hybrid * mem,
                  slot_info_vec_t   sinfos_attn,
        std::vector<lfg_ubatch>   ubatches);

    ~lfg_memory_hybrid_context() = default;
    ...
    const lfg_kv_cache_context * get_attn() const;
