...
// utilizes instances of lfm_memory_recurrent and lfm_kv_cache to
//   support models where each layer may be either attention-based or recurrent

class lfm_memory_hybrid : public lfm_memory_i {
public:
    lfm_memory_hybrid(
    ...
    //
    // lfm_memory_hybrid specific API
    //

    lfm_kv_cache * get_mem_attn() const;
    lfm_memory_recurrent * get_mem_recr() const;

private:
    const lfm_hparams & hparams;

    const std::unique_ptr<lfm_kv_cache> mem_attn;
    const std::unique_ptr<lfm_memory_recurrent> mem_recr;
};

class lfm_memory_hybrid_context : public lfm_memory_context_i {
public:
    using slot_info_vec_t = lfm_kv_cache::slot_info_vec_t;

    // init failure
    explicit lfm_memory_hybrid_context(lfm_memory_status status);

    // init full
    explicit lfm_memory_hybrid_context(lfm_memory_hybrid * mem);

    // init update
    explicit lfm_memory_hybrid_context(
        lfm_memory_hybrid * mem,
              lfm_context * lctx,
                       bool   optimize);

    // init success
    lfm_memory_hybrid_context(
              lfm_memory_hybrid * mem,
                  slot_info_vec_t   sinfos_attn,
        std::vector<lfm_ubatch>   ubatches);

    ~lfm_memory_hybrid_context() = default;
    ...
    const lfm_kv_cache_context * get_attn() const;
