...
// #include "models/models.h" // Removed external dependency
...
ggml_cgraph * clip_graph_conformer::build() {
    ...
    auto add_conv_bias = [&](ggml_tensor * conv_out, ggml_tensor * bias) {
        GGML_ASSERT(bias->ne[0] == conv_out->ne[2]);
        ggml_tensor * bias_4d = ggml_reshape_4d(ctx0, bias, 1, 1, bias->ne[0], 1);
        return ggml_add(ctx0, conv_out, bias_4d);
    };
    ...
    // pre encode, conv subsampling
    {
        ...
        cur = add_conv_bias(cur, model.pre_encode_conv_X_b[0]);
    ...
    for (int il = 0; il < hparams.n_layer; il++) {
        ...
        {
            auto * x = cur;
            ggml_tensor * pw1_w = layer.conv_pw1_w;
            if (ggml_n_dims(pw1_w) == 3) {
                pw1_w = ggml_reshape_2d(ctx0, pw1_w, pw1_w->ne[1], pw1_w->ne[2]);
            }
            ...
            x = ggml_pad(ctx0, x, 4, 0, 0, 0);
            ggml_tensor * conv_dw_w = layer.conv_dw_w;
            if (ggml_n_dims(conv_dw_w) == 3) {
                conv_dw_w = ggml_reshape_2d(ctx0, conv_dw_w, conv_dw_w->ne[0], conv_dw_w->ne[2]);
            }
            ...
            // pointwise_conv2
            ggml_tensor * pw2_w = layer.conv_pw2_w;
            if (ggml_n_dims(pw2_w) == 3) {
                pw2_w = ggml_reshape_2d(ctx0, pw2_w, pw2_w->ne[1], pw2_w->ne[2]);
            }
...
static ggml_cgraph * clip_image_build_graph(clip_ctx * ctx, const clip_image_f32_batch & imgs) {
    ...
    switch (ctx->proj_type()) {
        ...
        default:
            {
                 throw std::runtime_error("Unsupported projector type in simplified clip.cpp");
            }
    }
...
struct clip_model_loader {
    ...
    void load_tensors(clip_ctx & ctx_clip) {
        ...
        for (int il = 0; il < hparams.n_layer; ++il) {
            ...
            if (!layer.ln_1_w) {
                layer.ln_1_w = get_tensor(string_format(TN_ATTN_LN, prefix, il, "weight"), false);
            }
            layer.ln_2_w = get_tensor(string_format(TN_LN_2,        prefix, il, "weight"), false);
            if (!layer.ln_2_w) {
                layer.ln_2_w = get_tensor(string_format(TN_MLP_LN,  prefix, il, "weight"), false);
            }
            ...
            if (!layer.ln_1_b) {
                layer.ln_1_b = get_tensor(string_format(TN_ATTN_LN, prefix, il, "bias"), false);
            }
            layer.ln_2_b = get_tensor(string_format(TN_LN_2,        prefix, il, "bias"), false);
            if (!layer.ln_2_b) {
                layer.ln_2_b = get_tensor(string_format(TN_MLP_LN,  prefix, il, "bias"), false);
            }
            ...
            case PROJECTOR_TYPE_LFM2A:
                {
                    ...
                    for (int il = 0; il < hparams.n_layer; ++il) {
                        ...
                        layer.ff_norm_w   = get_tensor(string_format(TN_FFN_NORM,   prefix, il, "weight"), false);
                        if (!layer.ff_norm_w) {
                            layer.ff_norm_w = get_tensor(string_format(TN_ATTN_NORM, prefix, il, "weight"));
                        }
                        layer.ff_norm_b   = get_tensor(string_format(TN_FFN_NORM,   prefix, il, "bias"), false);
                        if (!layer.ff_norm_b) {
                            layer.ff_norm_b = get_tensor(string_format(TN_ATTN_NORM, prefix, il, "bias"), false);
                        }

                        layer.ff_norm_1_w = get_tensor(string_format(TN_FFN_NORM_1, prefix, il, "weight"), false);
                        ...
                        if (!layer.norm_conv_w) {
                            layer.norm_conv_w = get_tensor(string_format(TN_CONV_BN, prefix, il, "weight"), false);
                        }
                        ...
                        if (!layer.norm_conv_b) {
                            layer.norm_conv_b = get_tensor(string_format(TN_CONV_BN, prefix, il, "bias"), false);
                        }
                        ...
                        if (!layer.conv_norm_w) {
                            layer.conv_norm_w = get_tensor(string_format(TN_CONV_BN, prefix, il, "weight"), false);
                        }
                        layer.conv_norm_b  = get_tensor(string_format(TN_CONV_NORM, prefix, il, "bias"), false);
                        if (!layer.conv_norm_b) {
                            layer.conv_norm_b = get_tensor(string_format(TN_NORM_CONV, prefix, il, "bias"), false);
                        }
                        if (!layer.conv_norm_b) {
                            layer.conv_norm_b = get_tensor(string_format(TN_CONV_BN, prefix, il, "bias"), false);
                        }
                        layer.conv_dw_w    = get_tensor(string_format(TN_CONV_DW,   prefix, il, "weight"), false);
                        if (!layer.conv_dw_w) {
                            layer.conv_dw_w = get_tensor(string_format(TN_CONV_DW_DOT, prefix, il, "weight"));
                        }
                        layer.conv_dw_b    = get_tensor(string_format(TN_CONV_DW,   prefix, il, "bias"), false);
                        if (!layer.conv_dw_b) {
                            layer.conv_dw_b = get_tensor(string_format(TN_CONV_DW_DOT, prefix, il, "bias"));
                        }
                        layer.conv_pw1_w   = get_tensor(string_format(TN_CONV_PW1,  prefix, il, "weight"), false);
                        if (!layer.conv_pw1_w) {
                            layer.conv_pw1_w = get_tensor(string_format(TN_CONV_PW1_DOT, prefix, il, "weight"));
                        }
                        layer.conv_pw1_b   = get_tensor(string_format(TN_CONV_PW1,  prefix, il, "bias"), false);
                        if (!layer.conv_pw1_b) {
                            layer.conv_pw1_b = get_tensor(string_format(TN_CONV_PW1_DOT, prefix, il, "bias"));
                        }
                        layer.conv_pw2_w   = get_tensor(string_format(TN_CONV_PW2,  prefix, il, "weight"), false);
                        if (!layer.conv_pw2_w) {
                            layer.conv_pw2_w = get_tensor(string_format(TN_CONV_PW2_DOT, prefix, il, "weight"));
                        }
                        layer.conv_pw2_b   = get_tensor(string_format(TN_CONV_PW2,  prefix, il, "bias"), false);
                        if (!layer.conv_pw2_b) {
                            layer.conv_pw2_b = get_tensor(string_format(TN_CONV_PW2_DOT, prefix, il, "bias"));
                        }
                    }
                } break;
            default:
...
bool clip_image_batch_encode(clip_ctx * ctx, const int n_threads, const clip_image_f32_batch * imgs_c_ptr, float * vec) {
    ...
    auto set_input_f32 = [&get_inp_tensor](const char * name, std::vector<float> & values) {
        ...
        if (ggml_nelements(cur) != (int64_t)values.size()) {
            LOG_ERR("%s: input %s size mismatch (tensor=%" PRId64 ", values=%zu)\n",
                    __func__, name, ggml_nelements(cur), values.size());
        }
...
int32_t clip_get_audio_num_mel_bins(const struct clip_ctx * ctx) {
    return ctx->model.hparams.n_mel_bins;
}
