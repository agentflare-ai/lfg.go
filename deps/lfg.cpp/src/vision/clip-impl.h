...
#define TN_MLP_LN          "%s.blk.%d.mlp_ln.%s"
...
#define TN_ATTN_NORM       "%s.blk.%d.attn_norm.%s"
#define TN_CONV_PW1        "%s.blk.%d.conv_pw1.%s"
#define TN_CONV_PW2        "%s.blk.%d.conv_pw2.%s"
// lfm2 audio (newer naming)
#define TN_CONV_BN         "%s.blk.%d.conv.bn.%s"
#define TN_CONV_DW_DOT     "%s.blk.%d.conv.dw.%s"
#define TN_CONV_PW1_DOT    "%s.blk.%d.conv.pw1.%s"
#define TN_CONV_PW2_DOT    "%s.blk.%d.conv.pw2.%s"
...
static void clip_log_callback_default(enum ggml_log_level level, const char * text, void * user_data) {
    (void) user_data;
    switch (level) {
        case GGML_LOG_LEVEL_INFO:  spdlog::info("{}", text);  break;
        case GGML_LOG_LEVEL_WARN:  spdlog::warn("{}", text);  break;
        case GGML_LOG_LEVEL_ERROR: spdlog::error("{}", text); break;
        case GGML_LOG_LEVEL_DEBUG: spdlog::debug("{}", text); break;
        default:                   spdlog::info("{}", text);  break;
    }
}
...
#define LOG_INF(...) clip_log_internal(GGML_LOG_LEVEL_DEBUG, __VA_ARGS__)
...
#define LOG_CNT(...) clip_log_internal(GGML_LOG_LEVEL_DEBUG, __VA_ARGS__)
...
static void print_tensor_shape(ggml_tensor * t) {
    std::string s = string_format("%s.shape = [", t->name);
    for (int i = 0; i < ggml_n_dims(t); ++i) {
        s += string_format("%" PRId64, t->ne[i]);
